...
def parse_binary_xml_for_hashes(filepath):
    ...
    try:
        with open(filepath, "rb") as f:
            ...
            _user_version = read_u32()
            num_blocks = read_u32()
            num_block_types = read_u16()

            block_types = []
            for _ in range(num_block_types):
                length = read_u32()
                block_types.append(f.read(length))

            block_type_indices = [read_u16() for _ in range(num_blocks)]
            _block_sizes = [read_u32() for _ in range(num_blocks)]

            num_strings = read_u32()
            _max_string_len = read_u32()
            ...
            if (
                num_blocks != 1
                or b"xml::dom::CStreamableNode"
                not in block_types[block_type_indices[0]]
            ):
                return hashes

            _node_count = read_u32()
            _total_attr_count = read_u32()
            string_block_size = read_u32()

            # Skip string block, we only need hashes (Data)
            f.seek(string_block_size, 1)
...
def main():
    if len(sys.argv) < 2:
        ...
        print(
            'Example: python scan_binary_xml_hashes.py "C:\\Games\\Divinity2\\Data\\Public"'
        )
        return
